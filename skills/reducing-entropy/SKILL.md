...
description: Manual-only skill for minimizing total codebase size. Only activate when explicitly requested by user. Asks three questions for every change — smallest codebase solving the problem, does this reduce total code, what can be deleted. Identifies red flags like premature flexibility, status quo bias, and "better separation" that adds lines. Measures success by final code amount, not effort.
---
