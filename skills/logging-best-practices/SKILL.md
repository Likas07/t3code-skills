...
description: Logging best practices focused on wide events (canonical log lines) — Stripe-style structured logging that emits one context-rich event per request per service. Covers high-cardinality fields, business context inclusion, middleware patterns, and anti-patterns to avoid (scattered logs, unstructured strings). Use when setting up or improving application logging.
license: MIT
metadata:
  author: boristane
  version: "1.0.0"
